	...
	<script type="text/javascript">
		...
		function _role_cmp_save(editor, e){ 
			...
			if(App.empty(record.data.grant_type_)){
 			...
 			record.data.role_id_ = App.selectone(_g_role).data.id_;
			App.ajax({
  				params:record.data,
		...
		function _w_role_cmp_onshow(){
			var record = App.selectone(_g_role);
			var title = App.merge('角色页面元素授权 [当前角色：{0}]', record.data.name_);
			_w_role_cmp.setTitle(App.title(title));
			_t_module_cmp_load();
		...
		function _g_cmp_query(flag) {
	    	...
	    	var params = {
	    			role_id_ : App.selectone(_g_role).data.id_,
		...
		function _w_role_user_onshow(){
    		var record = App.selectone(_g_role);
    		var title = App.merge('角色用户授权 [当前角色：{0}]', record.data.name_);
    		_w_role_user.setTitle(App.title(title));
		...
		function _g_user_query() {
	    	...
	    	var record = App.selectone(_t_org_role);
			if(!App.empty(record)){
		...
		//取消角色用户授权
	 	function _role_user_cancel(){
				...
				App.ajax({
					...
					params:{
						_selection: App.selection(_g_user2, 'user_role_id_')
					},
					ok : function(data) {
						App.tip(data.appmsg);
    	...
    	function _w_role_module_show(){
	        ...
	        if(rows === 0){
	        	...
	        	App.tip('只能对单个角色进行授权操作，批量处理请选择其它相关功能。');
				return;
	        }
    		_w_role_module.show();
    	}
    	...
    	function _w_role_module_onshow(){
    		var record = App.selectone(_g_role);
    		var title = App.merge('角色菜单授权 [当前角色：{0}]', record.data.name_);
    		_w_role_module.setTitle(App.title(title));
		...
		function _g_role_del(){
			...
			var msg =  App.merge('确认要删除选中的[{0}]条数据吗？', rows);
    	...
    	function _fn_updateStatus(status_){
 				...
 				var msg =  App.merge('确认要停用选中的[{0}]个角色吗？', rows);
 				if(status_ === 1){
 					msg =  App.merge('确认要启用选中的[{0}]个角色吗？', rows);
 				}
 				App.confirm(msg, function(btn){
	 					if(btn === 'cancel'){
	 						App.tip('操作被取消。');
