...
<aos:onready>
	...
	<aos:window id="_w_postorg" title="批量修改岗位所属组织">
		<aos:formpanel id="_f_postorg" width="400" layout="anchor" labelWidth="70">
			<aos:hiddenfield name="_selection" fieldLabel="已选中的岗位ID集合" />
			<aos:hiddenfield name="org_id_" />
			<aos:triggerfield fieldLabel="所属组织" name="org_name_" allowBlank="false" editable="false" onfocus="_w_org_find_show"
				trigger1Cls="x-form-search-trigger" onTrigger1Click="_w_org_find_show" />
	...
	<script type="text/javascript">
		...
		function _post_cmp_save(editor, e){ 
			...
			if(App.empty(record.data.grant_type_)){
 			...
 			record.data.post_id_ = App.selectone(_g_post).data.id_;
			App.ajax({
  				params:record.data,
		...
		function _w_post_cmp_onshow(){
			var record = App.selectone(_g_post);
			var title = App.merge('岗位页面元素授权 [当前岗位：{0}]', record.data.name_);
			_w_post_cmp.setTitle(App.title(title));
			_t_module_cmp_load();
		...
		function _g_cmp_query(flag) {
	    	...
	    	var params = {
	    			post_id_ : App.selectone(_g_post).data.id_,
    	...
    	function _w_post_user_onshow(){
    		var record = App.selectone(_g_post);
    		var title = App.merge('岗位用户授权 [当前岗位：{0}]', record.data.name_);
    		_w_post_user.setTitle(App.title(title));
		...
		function _post_user_save(){
			...
			if(rows === 0){
				App.tip('操作被取消，请先选中要授权的用户。');
				return;
			}
			var record = App.selectone(_g_post);
			var params = {
				_selection : App.selection(_g_user, 'id_'),
        ...
        //取消岗位用户授权
	 	function _g_user2_del(){
 				var rows = App.rows(_g_user2);
 				if(rows === 0){
 					App.tip('操作被取消，请先选中要撤消授权的用户。');
 					return;
 				}
				App.ajax({
					url : 'delPostUserGrantInfo.jhtml',
					params:{
						_selection: App.selection(_g_user2, 'user_post_id_')
					},
					ok : function(data) {
						App.tip(data.appmsg);
    	...
    	function _w_post_module_onshow(){
    		var record = App.selectone(_g_post);
    		var title = App.merge('岗位菜单授权 [当前岗位：{0}]', record.data.name_);
    		_w_post_module.setTitle(App.title(title));
    	...
    	function _post_module_save(){
    		var _selection = App.selection(_t_module, 'id');
	   ...
	   //修改岗位保存
		function _f_post_u_save(){
			    ...
			    if(org_id_ != old_org_id_){
			    	App.confirm('提示：修改岗位所属组织将清除岗位和用户的关联关系。<br><br>继续保存吗？', function(btn){
							if(btn === 'cancel'){
								App.tip('岗位修改操作被取消。');
							}else{
								_f_post_u_save2();
	   ...
	   function _f_post_u_save2(){
        ...
        //删除岗位
	 	function _g_post_del(){
	 				var selection = App.selection(_g_post, 'id_');
	 				if(App.empty(selection)){
	 					App.tip('删除前请先选中数据。');
	 					return;
	 				}
	 				var rows = App.rows(_g_post);
	 				var msg =  App.merge('确认要删除选中的[{0}]个岗位吗？', rows);
    	...
    	function _fn_updateStatus(status_){
 				...
 				var msg =  App.merge('确认要停用选中的[{0}]个岗位吗？', rows);
 				if(status_ === 1){
 					msg =  App.merge('确认要启用选中的[{0}]个岗位吗？', rows);
 				}
 				var selection = App.selection(_g_post, 'id_');
 				App.confirm(msg, function(btn){
	 					if(btn === 'cancel'){
    	...
    	function _w_postorg_show(){
		    ...
		    var selection = App.selection(_g_post, 'id_');
		    _f_postorg.down('[name=_selection]').setValue(selection); 
    		_w_postorg.show();
    	}
    	
    	//批量修改岗位所属组织
    	function _f_postorg_save(){
    		if(!App.valid(_f_postorg)) return;
    		var rows = App.rows(_g_post);
    		var msg =  App.merge('提示：修改岗位所属组织将清除所选[{0}]个岗位和用户的关联关系。<br><br>继续保存吗？', rows);
	    	App.confirm(msg, function(btn){
				if(btn === 'cancel'){
					App.tip('操作被取消。');
					return;
