...
<aos:onready>
	...
	<script type="text/javascript">
		...
		function _w_org_u_show(){
			App.reset(_f_org_u);
			var record = App.selectone(_g_org);
			if(record){
				_w_org_u.show();
				...
				if(record.data.cascade_id_ === '0'){
						App.reads(_f_org_u, 'parent_id_');
				}else{
						App.edits(_f_org_u, 'parent_id_');
				}
	     	}
		}
	   ...
	   //修改组织保存
        ...
        //删除组织
	 	function _g_org_del(){
	 				var selection = App.selection(_g_org, 'id_');
	 				if(App.empty(selection)){
	 					App.tip('删除前请先选中数据。');
	 					return;
	 				}
	 				var rows = App.rows(_g_org);
	 				var msg =  App.merge('确认要删除选中的{0}个组织及其可能包含的子组织吗？', rows);
