...
<aos:onready>
			...
			<aos:docked forceBoder="0 0 1 0">
				...
				<aos:dockeditem text="更多操作" tooltip="更多业务操作" icon="icon140.png">
					<aos:menu>
						...
						<aos:menuitem xtype="menuseparator" />
						<aos:menuitem text="批量修改所属组织" onclick="_w_userorg_show" icon="icon87.png" />
					</aos:menu>
				</aos:dockeditem>
			</aos:docked>
			<aos:menu>
		...
		<aos:formpanel id="_f_user" width="700" layout="column">
			...
			<aos:fieldset title="基本信息" labelWidth="75">
				...
				<aos:textfield name="password_" fieldLabel="初始密码" allowBlank="false" inputType="password" maxLength="20"
					columnWidth="0.49" />
				<aos:combobox name="sex_" fieldLabel="性别" dicField="sex_" emptyText="请选择..." value="0" columnWidth="0.5" />
			...
			<aos:fieldset title="配置信息" labelWidth="80" border="true">
				...
				<aos:combobox name="layout_" fieldLabel="主布局" value="${user_cfg_.layout_}" dicField="layout_" columnWidth="0.33" />
				<aos:combobox name="nav_mode_" fieldLabel="导航模式" value="${user_cfg_.nav_mode_}" dicField="nav_mode_"
					columnWidth="0.33" />
				...
				<aos:combobox name="is_show_mac_nav_" fieldLabel="浮动导航" value="${user_cfg_.is_show_mac_nav_}"
					dicField="is_show_mac_nav_" columnWidth="0.33" />
				<aos:combobox name="nav_quick_layout_" fieldLabel="快捷菜单布局" dicField="nav_quick_layout_" labelWidth="100"
					value="${user_cfg_.nav_quick_layout_}" columnWidth="0.33" />
		...
		<aos:formpanel id="_f_user_u" width="700" layout="column" labelWidth="70">
			...
			<aos:fieldset title="基本信息" labelWidth="70" columnWidth="0.65" margin="0 10 0 0" height="${height}">
				<aos:triggerfield fieldLabel="所属组织" name="org_name_" allowBlank="false" editable="false" onfocus="_w_org_find_show"
					trigger1Cls="x-form-search-trigger" onTrigger1Click="_w_org_find_show" columnWidth="0.99" />
				<aos:textfield name="name_" fieldLabel="姓名" allowBlank="false" maxLength="50" columnWidth="0.99" />
				<aos:textfield name="account_" fieldLabel="登录帐号" allowBlank="false" maxLength="50" columnWidth="0.99" />
				<aos:combobox name="sex_" fieldLabel="性别" dicField="sex_" emptyText="请选择..." columnWidth="0.99" />
				<aos:textfield name="biz_code_" fieldLabel="业务对照码" maxLength="50" columnWidth="0.99" />
			...
			<aos:fieldset title="用户头像" labelWidth="70" columnWidth="0.35" height="${height}" contentEl="_div_photo" />
	...
	<aos:window id="_w_pwd" title="重置密码">
		<aos:formpanel id="_f_pwd" width="350" layout="anchor" labelWidth="75">
			<aos:hiddenfield name="_selection" fieldLabel="已选中的用户ID集合" />
	...
	<script type="text/javascript">
            ...
            //上传头像文件(注：文件上传操作不能使用App.Ajax()方法，只能使用Form自带的submit()函数)。
            function _f_upload_submit() {
                ...
                var params = {
                    id_:App.selectone(_g_user).data.id_,
                    bytearray_id_: _f_user_u.down('[name=bytearray_id_]').getValue()
                }
                App.wait();
                form.submit({
                    timeout: 60000, // 60s
            ...
            function _fn_del_user_photo(){
            	App.reset(_f_upload);
                App.confirm('提示：确认删除当前用户头像吗？', function (btn) {
                    if (btn === 'cancel') {
                        App.tip('用户操作被取消。');
                    } else {
                    	App.ajax({
                            forms: _f_user_u,
                            url: 'delUserPhoto.jhtml',
                            ok: function (data) {
                            	_w_upload.hide();
            ...
            function _g_cmpofuser_load() {
                var params = {
                    user_id_: App.selectone(_g_user).data.id_
                };
            ...
            function _user_cmp_save(editor, e) {
                var record = e.record;
                if (App.empty(record.data.grant_type_)) {
                    record.data.grant_type_ = '1';
                    e.record.commit();
                    return;
                }
                record.data.user_id_ = App.selectone(_g_user).data.id_;
                App.ajax({
                    params: record.data,
            ...
            function _w_user_cmp_onshow() {
                var record = App.selectone(_g_user);
                var title = App.merge('用户页面元素授权 [当前用户：{0}]', record.data.name_);
                _w_user_cmp.setTitle(App.title(title));
                _t_module_cmp_load();
            ...
            function _g_cmp_query(flag) {
                var record = App.selectone(_t_module_cmp);
                ...
                _g_cmp_store.load({
                    callback: function (records) {
                        if (records.length === 0) {
                            if (flag !== 1) App.tip('此功能模块没有绑定的页面元素，不能进行页面元素授权。');
                        }
                    }
                });
            }
            ...
            function _w_preview_onshow() {
                var record = App.selectone(_g_user);
                var title = App.merge('用户权限预览 [当前用户：{0}]', record.data.name_);
                _w_preview.setTitle(App.title(title));
            ...
            function _t_org_find_refresh() {
                var refreshnode = App.selectone(_t_org_find);
                if (!refreshnode) {
            ...
            function _t_org_find_select() {
                var record = App.selectone(_t_org_find);
            ...
            var _selection_t_module_init;
            var _selection_t_module_2_init;
            ...
            function _g_user_query() {
                ...
                var record = App.selectone(_t_org);
                if (!App.empty(record)) {
            ...
            function _t_org_refresh() {
                var refreshnode = App.selectone(_t_org);
                if (!refreshnode) {
            ...
            function _f_user_u_save() {
                ...
                if (org_id_ != old_org_id_) {
                    App.confirm('提示：所属组织被修改，继续保存将清除当前用户和岗位的授权关系。<br>继续保存吗？', function (btn) {
                        if (btn === 'cancel') {
                            App.tip('用户修改操作被取消。');
                        } else {
                            _f_user_u_save2();
            ...
            function _g_user_del() {
                var selection = App.selection(_g_user, 'id_');
                if (App.empty(selection)) {
                    App.tip('删除前请先选中数据。');
                    return;
                }
                var msg = App.merge('确认要删除选中的[{0}]个用户数据吗？', App.rows(_g_user));
            ...
            function _w_pwd_show() {
                ...
                var selection = App.selection(_g_user, 'id_');
                if (App.empty(selection)) {
                    App.tip('请先选中需要充值密码的用户数据。');
                    return;
                }
                _f_pwd.down('[name=_selection]').setValue(selection);
                _w_pwd.show();
            }
            ...
            function _f_pwd_save() {
                ...
                var msg = App.merge('确认要重置选中的[{0}]个用户的密码吗？', App.rows(_g_user));
                App.confirm(msg, function (btn) {
                    if (btn === 'cancel') {
                        App.tip('重置密码操作被取消。');
                        return;
            ...
            function _fn_updateStatus(status_) {
                var rows = App.rows(_g_user);
                if (rows === 0) {
                    App.tip('请先选择要操作的用户数据。');
                    return;
                }
                var msg = App.merge('确认要解封/启用选中的[{0}]个用户吗？', rows);
                if (status_ === 2) {
                    msg = App.merge('确认要停用选中的[{0}]个用户吗？', rows);
                }
                if (status_ === 3) {
                    msg = App.merge('确认要封锁选中的[{0}]个用户吗？', rows);
                }
                var selection = App.selection(_g_user, 'id_');
                App.confirm(msg, function (btn) {
                    if (btn === 'cancel') {
            ...
            function _w_userorg_show() {
                ...
                var selection = App.selection(_g_user, 'id_');
                _f_userorg.down('[name=_selection]').setValue(selection);
                _w_userorg.show();
            }

            //批量修改用户所属组织
            function _f_userorg_save() {
                if (!App.valid(_f_userorg)) return;
                var rows = App.rows(_g_user);
                var msg = App.merge('提示：修改所属组织将清除所选[{0}]个用户和岗位的授权关系。<br><br>继续保存吗？', rows);
                App.confirm(msg, function (btn) {
                    if (btn === 'cancel') {
                        App.tip('操作被取消。');
                        return;
            ...
            function _w_user_module_onshow() {
                var record = App.selectone(_g_user);
                var title = App.merge('用户菜单授权 [当前用户：{0}]', record.data.name_);
                _w_user_module.setTitle(App.title(title));
            ...
            function _user_module_save() {
                var _selection;
                if (_grant_type_ === 1) {
                    _selection = App.selection(_t_module, 'id');
                    if (_selection === _selection_t_module_init) {
                        App.tip('操作被取消，授权数据无变化不需要保存。');
                        return;
                    }
                } else {
                    _selection = App.selection(_t_module_2, 'id');
                    if (_selection === _selection_t_module_2_init) {
                        App.tip('操作被取消，授权数据无变化不需要保存。');
                        return;
                    }
                }
                ...
                App.ajax({
                    ...
                    ok: function (data) {
                        ...
                        if (_grant_type_ === 1) {
                            _t_module_1_load();
                            _selection_t_module_init = App.selection(_t_module, 'id');
                        } else {
                            _t_module_3_load();
                            _selection_t_module_2_init = App.selection(_t_module_2, 'id');
                        }
                    }
                });
            }
            ...
            function _w_user_post_onshow() {
                var record = App.selectone(_g_user);
                var title = App.merge('用户岗位授权 [当前用户：{0}]', record.data.name_);
                _w_user_post.setTitle(App.title(title));
            ...
            function _g_post_query() {
                ...
                var record = App.selectone(_t_org_post);
                if (!App.empty(record)) {
                    params.org_id_ = record.raw.id;
                } else {
                    params.org_id_ = '${dept_.id_}';
            ...
            function _user_post_save() {
                var rows = App.rows(_g_post);
                if (rows === 0) {
                    App.tip('操作被取消，请先选中要授权的岗位。');
                    return;
                }
                var record = App.selectone(_g_user);
                var params = {
                    _selection: App.selection(_g_post, 'id_'),
                    user_id_: record.data.id_
                };
                App.ajax({
                    params: params,
            ...
            function _user_post_cancel() {
                var rows = App.rows(_g_post2);
                if (rows === 0) {
                    App.tip('操作被取消，请先选中要撤消授权的岗位。');
                    return;
                }
                App.ajax({
                    url: 'delUserPostGrantInfo.jhtml',
                    params: {
                        _selection: App.selection(_g_post2, 'user_post_id_')
                    },
                    ok: function (data) {
                        App.tip(data.appmsg);
            ...
            function _w_user_role_onshow() {
                var record = App.selectone(_g_user);
                var title = App.merge('用户角色授权 [当前用户：{0}]', record.data.name_);
                _w_user_role.setTitle(App.title(title));
                _g_role_query();
                _g_role2_query();
            }
            ...
            function _user_role_save() {
                var rows = App.rows(_g_role);
                if (rows === 0) {
                    App.tip('操作被取消，请先选中要授权的角色。');
                    return;
                }
                var record = App.selectone(_g_user);
                var params = {
                    _selection: App.selection(_g_role, 'id_'),
                    user_id_: record.data.id_
                };
                App.ajax({
                    params: params,
            ...
            function _user_role_cancel() {
                var rows = App.rows(_g_role2);
                if (rows === 0) {
                    App.tip('操作被取消，请先选中要撤消授权的角色。');
                    return;
                }
                App.ajax({
                    url: 'delUserRoleGrantInfo.jhtml',
                    params: {
                        _selection: App.selection(_g_role2, 'user_role_id_')
                    },
                    ok: function (data) {
                        App.tip(data.appmsg);
