...
<aos:onready>
	<aos:viewport layout="border">
		<aos:treepanel id="_t_catalog" width="220" region="west" url="${cxt}/system/listCatalogs.jhtml"
			...
			nodeParam="parent_id_" bodyBorder="0 1 0 0" headerBorder="0 1 0 0" rootId="38">
	...
	<script type="text/javascript">
            ...
            function _w_index_u_show() {
                var record = App.selectone(_g_index);
                if (record) {
                    _w_index_u.show();
            ...
            function _w_dic_show() {
                App.reset(_f_dic);
                var record = App.selectone(_g_index, true);
                if (App.empty(record)) {
                    App.tip('请先在数据字典列表上选中1个数据字典。');
                    return;
                }
            ...
            function _w_dic_u_show() {
                var record = App.selectone(_g_dic);
                if (record) {
                    _w_dic_u.show();
                    _f_dic_u.loadRecord(record);
                    record = App.selectone(_g_index);
            ...
            //删除数据字典对照
            function _g_dic_del() {
                var selection = App.selection(_g_dic, 'id_');
                if (App.empty(selection)) {
                    App.tip('删除前请先选中数据。');
                    return;
                }
                var rows = App.rows(_g_dic);
                var msg = App.merge('确认要删除选中的[{0}]条数据字典对照明细吗？', rows);
            ...
            function _g_index_del() {
                var selection = App.selection(_g_index, 'id_');
                if (App.empty(selection)) {
                    App.tip('删除前请先选中数据。');
                    return;
                }
                var rows = App.rows(_g_index);
                var msg = App.merge('确认要删除选中的[{0}]条数据字典吗？', rows);
                App.confirm(msg, function (btn) {
                    if (btn === 'cancel') {
                        App.tip('删除操作被取消。');
                        return;
