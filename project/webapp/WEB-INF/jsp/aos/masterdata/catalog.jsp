...
<aos:onready>
	...
	<script type="text/javascript">
			...
			function _w_catalog_show(flag){
				App.reset(_f_catalog);
				var record = App.selectone(_t_catalog);
				_f_catalog.form.findField('flag').setValue(flag);
				if(flag === '1'){
					//新增科目
					_w_catalog.show();
					App.hides(_f_catalog, 'parent_id_,name_');
					App.disables(_f_catalog, 'parent_id_,name_');
					App.edits(_f_catalog, 'root_key_,root_name_');
				}else{
					//新增分类
					if(App.empty(record)){
						App.tip('操作被取消，新增分类前请先选中一个父分类。')
						return;
					}
					_w_catalog.show();
					_w_catalog.setTitle(App.title('新增分类'));
					App.shows(_f_catalog, 'parent_id_,name_');
					App.enables(_f_catalog, 'parent_id_,name_');
					...
					App.reads(_f_catalog, 'parent_id_,root_name_,root_key_');
				}
			}
		   ...
		   function _w_catalog_u_show(){
		  	 record = App.selectone(_t_catalog);
		  	 if(record){
			  	 _w_catalog_u.show();
		   ...
		   function _t_catalog_del(){
				...
				App.confirm('删除分类会导致和分类所属数据失去关联，请保证该分类下没有业务数据。<br><br>请谨慎操作，确认要删除吗？', function(btn){
					if(btn === 'cancel'){
						App.tip('删除操作被取消。');
						return;
					}
		    ...
		    //刷新
		    function fnrefresh(){
		    	var node = App.selectone(_t_catalog);
		    	if(node.isLeaf()){
		    		App.tip('操作被取消，叶子节点不能刷新。');
		    		return;
		    	}
