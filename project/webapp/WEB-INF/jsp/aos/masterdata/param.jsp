...
<aos:onready>
	<aos:viewport layout="border">
		<aos:treepanel id="_t_catalog" width="220" region="west" url="${cxt}/system/listCatalogs.jhtml"
			...
			nodeParam="parent_id_" bodyBorder="0 1 0 0" headerBorder="0 1 0 0" rootId="10">
	...
	<script type="text/javascript">
            ...
            function _w_param_u_show() {
                var record = App.selectone(_g_param);
                if (record) {
                    _w_param_u.show();
            ...
            function _g_param_del() {
                var selection = App.selection(_g_param, 'id_');
                if (App.empty(selection)) {
                    App.tip('删除前请先选中数据。');
                    return;
                }
                var rows = App.rows(_g_param);
            ...
            function _f_param_submit() {
                ...
                if (App.empty(overwrite_field_) && is_overwrite_ === '1') {
                    App.tip('覆盖字段为空的时候，是否覆盖只能选择否。请核对。');
                    return;
                }
                App.ajax({
                    forms: _f_param,
