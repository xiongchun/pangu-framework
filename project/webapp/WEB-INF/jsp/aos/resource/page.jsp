	...
	<script type="text/javascript">
		    ...
		    function _w_page_show(){
		    	...
		    	if(App.empty(record)){
		    		...
		    		if(record.data.leaf){
		    			...
		    			App.tip('操作被取消。只能在叶子节点上做新增页面操作，请重新选择。');
		    		}
		    	}
		    }
		  ...
		  function _f_page_save(){
			...
			function _w_page_u_show(){
					App.reset(_f_page_u);
	 				var record = App.selectone(_g_page);
	 				if(record.data.type_ === '1'){
	 					...
	 					if(record.data.type_ === '2'){
	 						App.edits(_f_page_u, 'enabled_,is_default_');
	 					}else if(record.data.type_ === '3'){
	 						App.reads(_f_page_u, 'enabled_,is_default_');
	 					}
					}
				}
		  ...
		  //删除页面组件
			function _g_page_del(){
				var rows = App.rows(_g_page);
				if(rows === 0){
					App.tip('删除前请先选中页面数据。');
					return;
				}
				var msg =  App.merge('确认要删除选中的[{0}]条页面记录吗？', rows);
				App.confirm(msg, function(btn){
					...
					App.ajax({
						...
						params:{
							_selection: App.selection(_g_page, 'id_')
						},
						ok : function(data) {
							App.tip(data.appmsg);
							_g_page_store.reload();
			...
			function _g_page_query() {
				...
				var record = App.selectone(_t_module);
				if(!App.empty(record) && record.data.id != '0'){
		    ...
		    function _t_module_find_refresh(){
				var refreshnode = App.selectone(_t_module_find);
				if (!refreshnode) {
		    ...
		    function _t_module_find_select(){
		    	var record = App.selectone(_t_module_find);
		    	if(!record.data.leaf){
		    		App.tip('操作被取消。只能选择叶子节点，请重新选择。');
		    		return;
		    	}
	    		//新增页面窗口
			...
			function _g_element_query2() {
				...
				var record_page_ = App.selectone(_g_page, true);
			...
			function _t_module_refresh() {
				var refreshnode = App.selectone(_t_module);
				if (!refreshnode) {
	    ...
	    function _w_element_show(){
	    	App.reset(_f_element);
	    	var rows = App.rows(_g_page);
	    	if(rows === 1){
		    	_w_element.show();
		    	var record = App.selectone(_g_page,true);
	    		...
	    		App.tip('操作被取消，请先选择一条页面记录。');
	    	}
	    }
	    
	    //弹出修改页面元素窗口
		function _w_element_u_show(){
				App.reset(_f_element_u);
 				var record = App.selectone(_g_element);
 				if(record){
 					_w_element_u.show();
		 ...
		 function _f_element_u_save(){
			...
			function _g_element_del(){
				var rows = App.rows(_g_element);
				if(rows === 0){
					App.tip('删除前请先选中页面数据。');
					return;
				}
				var msg =  App.merge('确认要删除选中的[{0}]条页面元素记录吗？', rows);
				App.confirm(msg, function(btn){
					if(btn === 'cancel'){
						App.tip('删除操作被取消。');
						return;
					...
					App.ajax({
						...
						params:{
							_selection: App.selection(_g_element, 'id_')
						},
						ok : function(data) {
							App.tip(data.appmsg);
							_g_element_store.reload();
