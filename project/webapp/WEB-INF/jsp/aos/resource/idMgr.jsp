...
<aos:onready>
	<aos:viewport layout="fit">
		<aos:gridpanel id="_g_id" url="listIds.jhtml" onrender="_g_id_query" onitemdblclick="_w_id_u_show">
			...
			<aos:column header="ID类型" dataIndex="type_" rendererField="type_" locked="true" />
	...
	<script type="text/javascript">
		...
		function _w_id_show(){
			//新增
			App.reset(_f_id);
			App.hides(_f_id, 'db_seq_name_');
			App.disables(_f_id, 'db_seq_name_');
			App.edits(_f_id, 'name_,type_');
			_f_id_init('1');
			_w_id.show();
		}
	  ...
	  function _w_id_u_show(){
	  	 record = App.selectone(_g_id);
	  	 if(record){
		  	 App.reads(_f_id, 'name_,type_');
		  	 _w_id.setTitle(App.title('修改ID'));
	  ...
	  function _f_id_init(type_){
			if(type_ === '1'){
				App.enables(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_circul_a,db_seq_name_,is_leftpad_,is_leftpad_a');
				App.shows(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_circul_a,db_seq_name_,is_leftpad_,is_leftpad_a');
				App.hides(_f_id, 'db_seq_name_');
				App.disables(_f_id, 'db_seq_name_');
			}
			if(type_ === '2'){
				App.disables(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_leftpad_a,db_seq_name_,is_leftpad_,is_leftpad_a');
				App.hides(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_circul_a,db_seq_name_,is_leftpad_,is_leftpad_a');
			}
			if(type_ === '3'){
				App.disables(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_circul_a,is_leftpad_,is_leftpad_a');
				App.hides(_f_id, 'min_value_,cur_value_,step_,max_value_,is_circul_,is_circul_a,is_leftpad_,is_leftpad_a');
				App.shows(_f_id, 'db_seq_name_');
				App.enables(_f_id, 'db_seq_name_');
			}
	  }
        ...
        //删除ID
		function _g_id_del(){
			var selection = App.selection(_g_id, 'id_');
			if(App.empty(selection)){
				App.tip('删除前请先选中数据。');
				return;
			}
			var rows = App.rows(_g_id);
        ...
        //计算ID值
		function fnCalc() {
			var record = App.selectone(_g_id);
			App.ajax({
				wait:false,
				url : 'calcId.jhtml',
				params:{
