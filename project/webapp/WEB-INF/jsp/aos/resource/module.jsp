...
<aos:onready>
	...
	<aos:window id="_w_module_find" title="上级菜单[双击选择]" height="-10" layout="fit" autoScroll="true"
		...
		<aos:treepanel id="_t_module_find" singleClick="false" width="320" bodyBorder="0 0 0 0" url="listModuleTree.jhtml"
			nodeParam="parent_id_" rootId="0" rootVisible="true" rootExpanded="false" rootText="${_root.name_}"
	...
	<script type="text/javascript">
			...
			function _w_module_u_show(){
				App.reset(_f_module_u);
 				var record = App.selectone(_g_module);
 				if(record){
 					_w_module_u.show();
 	 				...
 	 				if(record.data.cascade_id_ === '0'){
 	 					App.reads(_f_module_u, 'parent_id_');
 	 				}else{
 	 					App.edits(_f_module_u, 'parent_id_');
 	 				}
				}
			}
		...
		function _g_module_del(){
			var rows = App.rows(_g_module);
			if(rows === 0){
				App.tip('删除前请先选中数据。');
				return;
			}
			var msg =  App.merge('确认要删除选中的[{0}]个功能模块菜单数据吗？', rows);
			App.confirm(msg, function(btn){
				...
				App.ajax({
					...
					params:{
						_selection:  App.selection(_g_module, 'id_')
					},
					ok : function(data) {
						if(data.appcode === -1){
							App.err(data.appmsg);
