...
<aos:onready>
	...
	<script type="text/javascript">
		...
		//签收
		function _fn_claim() {
			var rec = App.selectone(_g_claim);
			var msg = App.merge('确认要签收任务[ID:{0}, 名称:{1}]吗？',
		...
		function _fn_unclaim(rec) {
			var msg = App.merge('确认要反签收任务[ID:{0}, 名称:{1}]吗？',
					rec.data.id_, rec.data.name_);
			App.confirm(msg, function(btn) {
				if (btn === 'cancel') {
					App.tip('操作被取消。');
		...
		function fn_unclaim_renderer(value, metaData, record) {
	    	if(App.empty(record.data.claim_time_)){
	    		return '-';
	    	}else{
