...
<aos:onready>
	...
	function _w_proc_u_show(){
		   var record = App.selectone(_g_procdef);
           if (record) {
        	   _w_proc_u.show();
               App.reset(_f_proc_u);
    ...
    function _t_module_click(){
    	...
    	if(!record.data.leaf && record.data.id !== '0'){
           ...
           function _fn_startProc() {
               var rec = App.selectone(_g_procdef);
               var msg = App.merge('确认要启动流程[ID:{0}, 名称:{1}]吗？', rec.data.proc_def_id_,rec.data.name_);
           ...
           function _fn_suspendProcDef() {
               var rec = App.selectone(_g_procdef); //右键菜单
               var msg = App.merge('确认要挂起流程[ID:{0}, 名称:{1}]吗？', rec.data.proc_def_id_,rec.data.name_);
               App.confirm(msg, function (btn) {
                   if (btn === 'cancel') {
                       App.tip('操作被取消。');
                       return;
           ...
           function _fn_activateProcDef() {
               var rec = App.selectone(_g_procdef); //右键菜单
               var msg = App.merge('确认要激活流程[ID:{0}, 名称:{1}]吗？', rec.data.proc_def_id_,rec.data.name_);
               App.confirm(msg, function (btn) {
                   if (btn === 'cancel') {
                       App.tip('操作被取消。');
                       return;
           ...
           function _g_procdef_del(cascade) {
               var rows = App.rows(_g_procdef);
               if (rows == 0) {
                   App.tip('删除前请先选中数据。');
                   return;
               }
               var msg = App.merge('确认要删除选中的[{0}]条流程数据吗？', rows);
               if(cascade === '1'){
            	   msg = App.merge('确认要级联删除选中的[{0}]条流程数据吗(含实例数据)？', rows);
               }
               App.confirm(msg, function (btn) {
                   if (btn === 'cancel') {
                       App.tip('删除操作被取消。');
                   ...
                   App.ajax({
                       ...
                       params: {
                           _selection: App.selection(_g_procdef, 'id_'),
                           cascade: cascade
                       },
                       ok: function (data) {
                           App.tip(data.appmsg);
