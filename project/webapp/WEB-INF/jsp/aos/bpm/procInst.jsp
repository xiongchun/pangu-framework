...
<aos:onready>
	...
	<script type="text/javascript">
		...
		function _g_inst_query() {
			var params = {
				procinst_status_: App.val('procinst_status_'),
				name_ : App.val('name_')
			};
	        var record = App.selectone(_g_procdef, true);
	        if(!App.empty(record)){
		...
		function _fn_suspendProcInst() {
			var rec = App.selectone(_g_inst); //右键菜单
			var msg = App.merge('确认要挂起流程实例[ID:{0}, 名称:{1}]吗？',
		...
		function _fn_activateProcInst() {
			var rec = App.selectone(_g_inst); //右键菜单
			var msg = App.merge('确认要激活流程实例[ID:{0}, 名称:{1}]吗？',
					rec.data.proc_inst_id_, rec.data.name_);
			App.confirm(msg, function(btn) {
				if (btn === 'cancel') {
					App.tip('操作被取消。');
		...
		function _fn_delProcInst() {
			var rows = App.rows(_g_inst);
			if (rows == 0) {
				App.tip('请先选中数据。');
				return;
			}
			var msg = App.merge('确认要终止选中的[{0}]条流程实例吗？', rows);
			App.confirm(msg, function(btn) {
				if (btn === 'cancel') {
					App.tip('结束操作被取消。');
					return;
				...
				App.ajax({
					...
					params : {
						_selection : App.selection(_g_inst, 'proc_inst_id_')
					},
					ok : function(data) {
						App.tip(data.appmsg);
						_g_inst_store.reload();
