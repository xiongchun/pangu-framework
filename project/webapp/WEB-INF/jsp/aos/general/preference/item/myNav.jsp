...
<aos:onready>
	...
	function _w_module_u_show(){
		var record = App.selectone(_g_nav);
		if(record){
			_w_module_u.show();
    ...
    //删除浮动菜单
	function _g_nav_del(){
		var rows = App.rows(_g_nav);
		if(rows === 0){
			App.tip('删除前请先选中数据。');
			return;
		}
		var msg =  App.merge('确认要删除选中的[{0}]条浮动菜单数据吗？', rows);
		App.confirm(msg, function(btn){
			...
			App.ajax({
				...
				params:{
					_selection:  App.selection(_g_nav, 'id_')
				},
				ok : function(data) {
					App.tip(data.appmsg);
					_g_nav_store.reload();
    ...
    //保存自定义快捷菜单
    var _selection_t_quick_init;
	App.job(function() {
		_selection_t_quick_init = App.selection(_t_quick, 'id');
	}, 1000);
    function _t_quick_save() {
        var _selection;
        _selection = App.selection(_t_quick, 'id');
        if (_selection === _selection_t_quick_init) {
            App.tip('操作被取消，数据无变化不需要保存。');
            return;
        }
        var params = {
    ...
    function _g_module_sort_save(editor, e) {
        var record = e.record;
        if (App.empty(record.data.sort_no_)) {
            e.record.commit();
            return;
        }
        App.ajax({
