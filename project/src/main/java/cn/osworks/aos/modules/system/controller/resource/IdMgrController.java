...
import cn.osworks.aos.modules.system.service.resource.IdMgrService;
...
public class IdMgrController {
	...
	public void listIds(HttpServletRequest request, HttpServletResponse response) {
		...
		List<Aos_ge_sequencePO> list = aos_ge_sequenceMapper.likePage(pDto);
		for (Aos_ge_sequencePO aos_ge_sequencePO : list) {
			if (DicCons.SEQUENCE_TYPE_UUID.equals(aos_ge_sequencePO.getType_()) || DicCons.SEQUENCE_TYPE_DBSEQUENCE.equals(aos_ge_sequencePO.getType_())) {
				aos_ge_sequencePO.setStart_(null);
				aos_ge_sequencePO.setStep_(null);
				aos_ge_sequencePO.setDb_seq_name_(null);
				aos_ge_sequencePO.setMax_value_(null);
				aos_ge_sequencePO.setMin_value_(null);
			}
		}
