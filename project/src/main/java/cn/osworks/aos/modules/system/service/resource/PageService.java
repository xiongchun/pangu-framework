...
import cn.osworks.aos.modules.system.dao.mapper.Aos_au_page_el_grantMapper;
...
import cn.osworks.aos.modules.system.dao.po.Aos_au_page_el_grantPO;
import cn.osworks.aos.modules.system.dao.vo.PageVO;
...
public class PageService {
	...
	@Autowired
	private Aos_au_page_el_grantMapper aos_au_page_el_grantMapper;
	@Autowired
	...
	public List<Dto> listElements4Mgr(Dto inDto) {
		...
		for (Dto dto : list) {
			if (StringUtils.equals(dto.getString("page_id_"), dto.getString("module_id_"))) {
				//主页面
				Aos_au_modulePO aos_au_modulePO = aos_au_moduleMapper.selectByKey(dto.getString("module_id_"));
				if (AOSUtils.isNotEmpty(aos_au_modulePO)) {
					dto.put("page_name_", aos_au_modulePO.getName_() + "-主页面");
				}
			}
		}
		return list;
	...
	public Dto savePage(Dto inDto) {
		...
		if (StringUtils.equals(inDto.getString("type_"), DicCons.PAGE_TYPE_SUB)
			...
			int rows = aos_au_pageMapper.rows(qDto);
			if (rows > 0) {
		...
		Aos_au_pagePO aos_au_pagePO = new Aos_au_pagePO();
		AOSUtils.apply(inDto, aos_au_pagePO);
		aos_au_pagePO.setId_(AOSId.id(IdCons.GID));
		aos_au_pageMapper.insert(aos_au_pagePO);
	...
	public Dto updatePage(Dto inDto) {
		...
		if (StringUtils.equals(inDto.getString("type_"), DicCons.PAGE_TYPE_SUB)
				&& StringUtils.equals(inDto.getString("is_default_"), AOSCons.YES)) {
			// 子页面才做这些判断
			Aos_au_pagePO oldPO = aos_au_pageMapper.selectByKey(inDto.getString("id_"));
		...
		Aos_au_pagePO aos_au_pagePO = new Aos_au_pagePO();
		AOSUtils.apply(inDto, aos_au_pagePO);
		aos_au_pageMapper.updateByKey(aos_au_pagePO);
	...
	public Dto deletePage(Dto qDto) {
		...
		for (String page_id_ : selections) {
			Aos_au_pagePO aos_au_pagePO = aos_au_pageMapper.selectByKey(page_id_);
			if (AOSUtils.isEmpty(aos_au_pagePO)) {
				cancel++;
				continue;
			}
			List<Aos_au_page_elPO> aos_au_page_elPOs = aos_au_page_elMapper.list(Dtos.newDto("page_id_", page_id_));
			for (Aos_au_page_elPO aos_au_page_elPO : aos_au_page_elPOs) {
				...
				// 清除页面元素
				aos_au_page_elMapper.deleteByKey(aos_au_page_elPO.getId_());
			}
			// 清除页面
			aos_au_pageMapper.deleteByKey(page_id_);
			del++;
		}
	...
	public Dto saveElement(Dto inDto) {
		...
		int rows = aos_au_page_elMapper.rows(calcDto);
		if (rows > 0) {
			outDto.setAppCode(AOSCons.ERROR);
			outDto.setAppMsg("当前页面元素Dom标识已被占用，请重新输入。");
			return outDto;
		}
		Aos_au_page_elPO aos_au_page_elPO = new Aos_au_page_elPO();
		AOSUtils.apply(inDto, aos_au_page_elPO);
		aos_au_page_elPO.setId_(AOSId.id(IdCons.GID));
		aos_au_page_elMapper.insert(aos_au_page_elPO);
	...
	public Dto updateElement(Dto inDto) {
		Dto outDto = Dtos.newOutDto();
		Aos_au_page_elPO aos_au_page_elPO = aos_au_page_elMapper.selectByKey(inDto.getString("id_"));
		if (!inDto.getString("dom_id_").equals(aos_au_page_elPO.getDom_id_())) {
			// 修改了页面元素ID属性
			Dto calcDto = Dtos.newCalcDto("COUNT(id_)");
			calcDto.put("dom_id_", inDto.getString("dom_id_"));
			calcDto.put("module_id_", inDto.getString("module_id_"));
		...
		Aos_au_page_elPO aos_au_page_elPO2 = new Aos_au_page_elPO();
		AOSUtils.apply(inDto, aos_au_page_elPO2);
		aos_au_page_elMapper.updateByKey(aos_au_page_elPO2);
	...
	public Dto deleteElement(Dto qDto) {
		...
		for (String id_ : selections) {
			aos_au_page_elMapper.deleteByKey(id_);
			// 清除授权资源
			sqlDao.delete("Resource.deleteAos_au_page_el_grant", id_);
		}
	...
	public List<Dto> listElement4Grant(Dto inDto) {
		...
		for (Dto elementDto : elementInfos) {
			...
			Aos_au_page_el_grantPO aos_au_page_el_grantPO = aos_au_page_el_grantMapper.selectOne(qDto);
			if (AOSUtils.isEmpty(aos_au_page_el_grantPO)) {
				elementDto.put("grant_type_", DicCons.ELEMENT_GRANT_TYPE_WSQ);
			} else {
				elementDto.put("grant_type_", aos_au_page_el_grantPO.getGrant_type_());
				elementDto.put("element_grant_id_", aos_au_page_el_grantPO.getId_());
			}
			//处理主页面时的所属页面
	...
	public void saveElementGrantInfo(Dto pDto) {
		Aos_au_page_el_grantPO aos_au_page_el_grantPO = new Aos_au_page_el_grantPO();
		AOSUtils.apply(pDto, aos_au_page_el_grantPO);
		String grant_type_ = pDto.getString("grant_type_");
		aos_au_page_el_grantPO.setGrant_type_(grant_type_);
		aos_au_page_el_grantPO.setEl_id_(pDto.getString("id_"));
		aos_au_page_el_grantPO.setId_(pDto.getString("element_grant_id_"));
		// 新授权
		if (AOSUtils.isEmpty(pDto.getString("element_grant_id_"))) {
			...
			if (!StringUtils.equals(DicCons.ELEMENT_GRANT_TYPE_CXSQ, grant_type_)) {
				aos_au_page_el_grantPO.setId_(AOSId.id(IdCons.RID));
				aos_au_page_el_grantMapper.insert(aos_au_page_el_grantPO);
			}
		} else {
			if (StringUtils.equals(DicCons.ELEMENT_GRANT_TYPE_CXSQ, grant_type_)) {
				// 撤销授权
				aos_au_page_el_grantMapper.deleteByKey(aos_au_page_el_grantPO.getId_());
			} else {
				// 修改授权类型
				aos_au_page_el_grantMapper.updateByKey(aos_au_page_el_grantPO);
			}
		}
	}
