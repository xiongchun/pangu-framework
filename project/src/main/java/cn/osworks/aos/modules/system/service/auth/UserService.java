...
import cn.osworks.aos.base.asset.AOSCodec;
...
import cn.osworks.aos.modules.system.dao.mapper.Aos_au_module_userMapper;
import cn.osworks.aos.modules.system.dao.mapper.Aos_au_orgMapper;
import cn.osworks.aos.modules.system.dao.mapper.Aos_au_pageMapper;
...
import cn.osworks.aos.modules.system.dao.po.Aos_au_module_userPO;
...
import cn.osworks.aos.modules.system.service.resource.ByteObjService;
...
public class UserService {
	...
	@Autowired
	private Aos_au_module_userMapper aos_au_module_userMapper;
	@Autowired
	...
	@Autowired 
	private Aos_au_pageMapper aos_au_pageMapper;
	...
	public String saveUserPhoto(Dto inDto) throws IOException {
		...
		if (AOSUtils.isEmpty(bytearray_id_)) {
			...
			aos_au_user_extPO.setBytearray_id_(bytearray_id_);
			aos_au_user_extMapper.updateByKey(aos_au_user_extPO);
		} else {
			// 修改上传
			Aos_ge_bytearrayPO aos_ge_bytearrayPO = aos_ge_bytearrayMapper.selectByKey(bytearray_id_);
	...
	public void delUserPhoto(Dto inDto) {
		...
		Aos_au_user_extPO aos_au_user_extPO = aos_au_user_extMapper.selectByKey(user_id_);
		aos_ge_bytearrayMapper.deleteByKey(aos_au_user_extPO.getBytearray_id_());
		aos_au_user_extPO.setBytearray_id_("");
		aos_au_user_extMapper.updateByKey(aos_au_user_extPO);
	}
	...
	public Dto getUser(Dto inDto) {
		...
		Aos_au_userPO aos_au_userPO = aos_au_userMapper.selectByKey(id_);
		aos_au_userPO.setPassword_(null); // 保护字段
		outDto.putAll(aos_au_userPO.toDto());
		Aos_au_user_extPO aos_au_user_extPO = aos_au_user_extMapper.selectByKey(id_);
		outDto.putAll(aos_au_user_extPO.toDto());
		Aos_au_user_cfgPO aos_au_user_cfgPO = aos_au_user_cfgMapper.selectByKey(id_);
		outDto.putAll(aos_au_user_cfgPO.toDto());
		// 查询所属组织
		Aos_au_orgPO aos_au_orgPO = aos_au_orgMapper.selectByKey(aos_au_userPO.getOrg_id_());
		outDto.put("org_name_", aos_au_orgPO.getName_());
		return outDto;
	}
	...
	public Dto saveUser(Dto inDto) {
		...
		if (!checkAccount(aos_au_userPO.getAccount_())) {
			outDto.setAppCode(AOSCons.ERROR);
			String msg = AOSUtils.merge("登录帐号[{0}]已被其它用户占用, 请重新输入。", aos_au_userPO.getAccount_());
			outDto.setAppMsg(msg);
			return outDto;
		}
		aos_au_userPO.setId_(AOSId.id(IdCons.USERID));
		UserInfoVO userInfoVO = inDto.getUserInfo();
		aos_au_userPO.setCreater_id_(userInfoVO.getId_());
		aos_au_userPO.setCreate_time_(AOSUtils.getDateTimeStr());
		Aos_au_orgPO aos_au_orgPO = aos_au_orgMapper.selectByKey(aos_au_userPO.getOrg_id_());
		aos_au_userPO.setOrg_cascade_id_(aos_au_orgPO.getCascade_id_());
		String password = encryptPwd(aos_au_userPO.getPassword_());
		aos_au_userPO.setPassword_(password);
		aos_au_userPO.setStatus_(DicCons.USER_STATUS_NORMAL);
		aos_au_userPO.setType_(DicCons.USER_TYPE_DEFAULT);
		aos_au_userMapper.insert(aos_au_userPO);

		// 写用户扩展表
		Aos_au_user_extPO aos_au_user_extPO = new Aos_au_user_extPO();
		AOSUtils.apply(inDto, aos_au_user_extPO);
		aos_au_user_extPO.setId_(aos_au_userPO.getId_());
		aos_au_user_extMapper.insert(aos_au_user_extPO);

		// 写用户配置信息表
		Aos_au_user_cfgPO aos_au_user_cfgPO = new Aos_au_user_cfgPO();
		AOSUtils.apply(inDto, aos_au_user_cfgPO);
		aos_au_user_cfgPO.setId_(aos_au_userPO.getId_());
		aos_au_user_cfgMapper.insert(aos_au_user_cfgPO);
	...
	public boolean checkAccount(String account_) {
		...
		Integer countInteger = Integer.valueOf(aos_au_userMapper.calc(calcDto));
		boolean out = true;
		if (countInteger > 0) {
			out = false;
		}
	...
	public Dto updateUser(Dto inDto) {
		...
		Aos_au_userPO aos_au_userPO = new Aos_au_userPO();
		AOSUtils.apply(inDto, aos_au_userPO);
		Aos_au_userPO oldAos_au_userPO = aos_au_userMapper.selectByKey(aos_au_userPO.getId_());
		if (!aos_au_userPO.getAccount_().equalsIgnoreCase(oldAos_au_userPO.getAccount_())) {
			if (!checkAccount(aos_au_userPO.getAccount_())) {
				outDto.setAppCode(AOSCons.ERROR);
				String msg = AOSUtils.merge("登录帐号[{0}]已被其它用户占用, 请重新输入。", aos_au_userPO.getAccount_());
				outDto.setAppMsg(msg);
		...
		if (!inDto.getString("org_id_").equals(inDto.getString("old_org_id_"))) {
			...
			aos_au_userPO.setOrg_cascade_id_(aos_au_orgPO.getCascade_id_());
			// 删除用户-岗位关联表
			sqlDao.delete("Auth.deleteAos_au_user_postByUser_id_", aos_au_userPO.getId_());
		}
		// 更新用户基本信息
	...
	public Dto deleteUser(Dto qDto) {
		...
		for (String id_ : selections) {
			Aos_au_userPO aos_au_userPO = aos_au_userMapper.selectByKey(id_);
			if (aos_au_userPO.getType_().equals(DicCons.USER_TYPE_SUPER)) {
				cancel++;
				continue;
			}
			// 清除和用户相关的资源
			resetWhenDeleteUser(id_);
			aos_au_userMapper.deleteByKey(id_);
			del++;
		}
		String msg = "操作完成, ";
		if (del > 0) {
	...
	private void resetWhenDeleteUser(String user_id_) {
		// 删除用户-角色关联表
		sqlDao.delete("Auth.deleteAos_au_user_roleByUser_id_", Dtos.newDto("user_id_", user_id_));
		// 删除用户-岗位关联表
		sqlDao.delete("Auth.deleteAos_au_user_postByUser_id_", user_id_);
		// 删除用户-菜单关联表
		sqlDao.delete("Auth.deleteAos_au_module_userByUser_id_", user_id_);
		// 删除用户-Mac菜单关联表
		sqlDao.delete("Auth.deleteAos_au_module_user_macstyleByUser_id_", user_id_);
		// 删除用户-页面元素关联表
		sqlDao.delete("Auth.deleteAos_au_element_grantByUser_id_", user_id_);
		// 删除用户配置表
		aos_au_user_cfgMapper.deleteByKey(user_id_);
		// 删除用户扩展表
		aos_au_user_extMapper.deleteByKey(user_id_);
	}
	...
	public Dto updateMyPwd(Dto inDto) {
		...
		Aos_au_userPO userPO = aos_au_userMapper.selectByKey(inDto.getString("id_"));
		...
		Aos_au_userPO aos_au_userPO = new Aos_au_userPO();
		aos_au_userPO.setId_(inDto.getString("id_"));
		aos_au_userPO.setPassword_(encryptPwd(inDto.getString("new_password_")));
		aos_au_userMapper.updateByKey(aos_au_userPO);
	...
	public void updateStatus(Dto qDto) {
		String[] selections = qDto.getSelection();
		Aos_au_userPO aos_au_userPO = new Aos_au_userPO();
		aos_au_userPO.setStatus_(qDto.getString("status_"));
		for (String aos_au_usera0 : selections) {
			aos_au_userPO.setId_(aos_au_usera0);
			aos_au_userMapper.updateByKey(aos_au_userPO);
		}
	}
	...
	public void updateUserOrg(Dto qDto) {
		String[] selections = qDto.getSelection();
		Aos_au_userPO aos_au_userPO = new Aos_au_userPO();
		aos_au_userPO.setOrg_id_(qDto.getString("org_id_"));
		for (String id_ : selections) {
			aos_au_userPO.setId_(id_);
			Aos_au_orgPO aos_au_orgPO = aos_au_orgMapper.selectByKey(aos_au_userPO.getOrg_id_());
			aos_au_userPO.setOrg_cascade_id_(aos_au_orgPO.getCascade_id_());
			aos_au_userMapper.updateByKey(aos_au_userPO);
			// 删除用户-岗位关联表
			sqlDao.delete("Auth.deleteAos_au_user_postByUser_id_", id_);
		}
	}
	...
	public void saveUserModuleGrantInfo(Dto pDto) {
		...
		// 每次授权都将历史数据清零
		sqlDao.delete("Auth.deleteAos_au_module_userByDto", delDto);
		String[] selections = pDto.getSelection();
		Aos_au_module_userPO aos_au_module_userPO = new Aos_au_module_userPO();
		for (String module_id_ : selections) {
			aos_au_module_userPO.setId_(AOSId.id(IdCons.RID));
			aos_au_module_userPO.setModule_id_(module_id_);
			aos_au_module_userPO.setUser_id_(pDto.getString("user_id_"));
			aos_au_module_userPO.setGrant_type_(pDto.getString("grant_type_"));
			aos_au_module_userPO.setOperate_time_(AOSUtils.getDateTimeStr());
			aos_au_module_userPO.setOperator_id_(userInfoVO.getId_());
			aos_au_module_userMapper.insert(aos_au_module_userPO);
		}
	}
	...
	public String getModuleTree4Select(Dto inDto) {
		...
		if (DicCons.USER_TYPE_SUPER.equals(userInfoVO.getType_())) {
			...
			aos_au_modulePOs = systemService.getAdminModulesOfUser(grantDto);
		}
	...
	private String toTreeModal(List<Aos_au_modulePO> aos_au_modulePOs, Dto inDto) {
		...
		Dto aos_au_module_userDto = Dtos.newDto();
		aos_au_module_userDto.put("user_id_", inDto.getString("user_id_")); // 界面选中的用户
		aos_au_module_userDto.put("grant_type_", inDto.getString("grant_type_"));
		List<Aos_au_module_userPO> aos_au_module_userPOs = aos_au_module_userMapper.list(aos_au_module_userDto);
		for (Aos_au_modulePO aos_au_modulePO : aos_au_modulePOs) {
			...
			// 可以改成AOSList的写法
			for (Aos_au_module_userPO aos_au_module_userPO : aos_au_module_userPOs) {
				if (aos_au_module_userPO.getModule_id_().equals(aos_au_modulePO.getId_())) {
					checked = true;
					break;
				}
			}
	...
	public Dto getModuleTree4Selected(Dto inDto) {
		Dto outDto = Dtos.newDto();
		inDto.put("status_", DicCons.ENABLED_YES);
		List<Aos_au_modulePO> aos_au_modulePOs = sqlDao.list("Auth.listUserModuleSelected", inDto);
	...
	public void saveUserPostGrantInfo(Dto pDto) {
		...
		for (String post_id_ : selections) {
			...
			aos_au_user_postPO.setPost_id_(post_id_);
			aos_au_user_postPO.setUser_id_(pDto.getString("user_id_"));
	...
	public Dto delUserPostGrantInfo(Dto qDto) {
		...
		for (String user_post_id_ : selections) {
			aos_au_user_postMapper.deleteByKey(user_post_id_);
		}
	...
	public void saveUserRoleGrantInfo(Dto pDto) {
		...
		for (String role_id_ : selections) {
			...
			aos_au_user_rolePO.setRole_id_(role_id_);
			aos_au_user_rolePO.setUser_id_(pDto.getString("user_id_"));
	...
	public List<Dto> listRoles4Grant(Dto inDto) {
		...
		if (StringUtils.equals(AOSCons.ROLE_GRANT_MODE_NOCASCADE, role_grant_mode)) {
			...
			Aos_au_orgPO aos_au_orgPO = aos_au_orgMapper.selectByKey(inDto.getUserInfo().getOrg_id_());
			inDto.put("creater_org_cascade_id_", aos_au_orgPO.getCascade_id_());
		} else {
			inDto.put("creater_org_id_", inDto.getUserInfo().getOrg_id_());
		}
	...
	public String getBizModulesOfUser(Dto inDto) {
		List<Aos_au_modulePO> aos_au_modulePOs = systemService.getBizModulesOfUser(inDto);
		return toTreeModal(aos_au_modulePOs);
	}
	...
	public String getAdminModulesOfUser(Dto inDto) {
		List<Aos_au_modulePO> aos_au_modulePOs = systemService.getAdminModulesOfUser(inDto);
		return toTreeModal(aos_au_modulePOs);
	}
	...
	public List<ElementVO> getElementsOfUser(Dto inDto) {
		...
		for (ElementVO elementVO : elementVOs) {
			if (StringUtils.equals(elementVO.getModule_id_(), elementVO.getPage_id_())) {
				...
				Aos_au_pagePO aos_au_pagePO = aos_au_pageMapper.selectByKey(elementVO.getPage_id_());
				elementVO.setPage_name_(aos_au_pagePO.getName_());
			}
		}
		return elementVOs;
	}
	 ...
	 * @return
	 */
	private String toTreeModal(List<Aos_au_modulePO> aos_au_modulePOs) {
		List<TreeNode> treeNodes = Lists.newArrayList();
		for (Aos_au_modulePO aos_au_modulePO : aos_au_modulePOs) {
			TreeNode treeNode = new TreeNode();
			treeNode.setId(aos_au_modulePO.getId_());
