...
import cn.osworks.aos.modules.system.service.IndexService;
...
public class IndexController {
	...
	public String login(HttpSession session, HttpServletRequest request) {
		...
		if (StringUtils.equals(login_back_img, "-1")) {
			int backImgIndex = RandomUtils.nextInt(4); // [0,4)
	...
	private Dto checkAccount(Dto inDto, HttpSession session) {
		Dto outDto = Dtos.newDto();
		// 帐号存在校验
		Dto aos_au_userDto = Dtos.newDto("account_", inDto.getString("account_"));
		Aos_au_userPO aos_au_userPO = aos_au_userMapper.selectOne(aos_au_userDto);
		if (AOSUtils.isEmpty(aos_au_userPO)) {
			outDto.setBooleanA(false);
			outDto.setAppCode(-2);
			...
			if (!StringUtils.equals(password, aos_au_userPO.getPassword_())) {
				outDto.setBooleanA(false);
				outDto.setAppCode(-3);
				outDto.setAppMsg("密码输入错误，请重新输入。");
			} else {
				// 有效状态校验
				if (!aos_au_userPO.getStatus_().equals(DicCons.USER_STATUS_NORMAL)) {
					outDto.setBooleanA(false);
					outDto.setAppCode(-4);
					outDto.setAppMsg("该帐号已停用，系统拒绝登录。");
				} else {
					outDto.setBooleanA(true);
					outDto.put("aos_au_userPO", aos_au_userPO);
				}
			}
		}
		return outDto;
	...
	public void doLogin(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		// 检查通过
		Dto outDto = Dtos.newDto();
		Aos_au_userPO aos_au_userPO = (Aos_au_userPO) checkAccountDto.get("aos_au_userPO");
		UserInfoVO userInfoVO = new UserInfoVO();
		AOSUtils.apply(aos_au_userPO, userInfoVO);
		Aos_au_user_cfgPO aos_au_user_cfgPO = WebCxt.getUserCfgInfo(aos_au_userPO.getId_());
		userInfoVO.setCfgInfo(aos_au_user_cfgPO);
		Aos_au_orgPO aos_au_orgPO = aos_au_orgMapper.selectByKey(aos_au_userPO.getOrg_id_());
		userInfoVO.setOrgInfo(aos_au_orgPO);
	...
	public void doDevLogin(HttpServletResponse response, HttpSession session) {
		Dto outDto = Dtos.newDto();
		String runas = WebCxt.getCfgOfXML("runas");
		// 生产模式下禁用开发者快捷登录功能
		if (AOSCons.RUNAS_PRO.equals(runas)) {
			outDto.setAppCode(-1);
			...
			if (!StringUtils.equals(dev_account_login, AOSCons.YES)) {
				...
				Dto aos_au_userDto = Dtos.newDto("account_", dev_account);
				Aos_au_userPO aos_au_userPO = aos_au_userMapper.selectOne(aos_au_userDto);
				UserInfoVO userInfoVO = new UserInfoVO();
				AOSUtils.apply(aos_au_userPO, userInfoVO);
				Aos_au_user_cfgPO aos_au_user_cfgPO = WebCxt.getUserCfgInfo(aos_au_userPO.getId_());
	...
	public String index(HttpServletRequest request, HttpServletResponse response, HttpSession session) {
		...
		if (StringUtils.equals(nav_quick_layout_, AOSCons.NAV_QUICK_LAYOUT_FLAT)) {
			...
			List<Aos_au_modulePO> aos_au_modulePOs = sqlDao.list("Auth.listUserQuickModuleSelected4Flat", inDto);
			request.setAttribute("aos_au_modulePOs", aos_au_modulePOs);
		}
		// 一些首页所需的页面变量
		initIndexVars(request, session);
	 ...
	 * @param request
	 */
	private void initIndexVars(HttpServletRequest request, HttpSession session) {
		...
		String nav_mode_ = WebCxt.getCfgByUser(session, "nav_mode_");
		navDto.put("nav_mode_", nav_mode_);
		String is_show_top_nav_ = WebCxt.getCfgByUser(session, "is_show_top_nav_");
		// 导航模式2只能强制显示水平导航条
		if (nav_mode_.equals("2")) {
