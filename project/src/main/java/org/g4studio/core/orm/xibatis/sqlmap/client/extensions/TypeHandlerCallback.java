 ...
 * <p/>
 * Using this interface, you can implement a type handler that will perform
 * customized processing before parameters are set on a PreparedStatement and
 * after values are retrieved from a ResultSet. Using a custom type handler you
 * can extend the framework to handle types that are not supported, or handle
 * supported types in a different way. For example, you might use a custom type
 * handler to implement proprietary BLOB support (e.g. Oracle), or you might use
 * it to handle booleans using "Y" and "N" instead of the more typical 0/1.
 * <p/>
 * <b>EXAMPLE</b>
 * <p>
...
public interface TypeHandlerCallback {
	...
	/**
	 * Performs processing on a value before it is used to set the parameter of
	 * a PreparedStatement.
	 * 
	 * @param setter
	...
	/**
	 * Performs processing on a value before after it has been retrieved from a
	 * ResultSet.
	 * 
	 * @param getter
	 ...
	 * type handler. This method is used to translate nullValue values into
	 * types that can be appropriately compared. If your custom type handler
	 * cannot support nullValues, or if there is no reasonable string
