...
/**
 * A thread safe client for working with your SQL Maps (Start Here). This
 * interface inherits transaction control and execution methods from the
 * SqlMapTransactionManager and SqlMapExecutor interfaces.
 * <p/>
 * The SqlMapClient is the central class for working with SQL Maps. This class
 * will allow you to run mapped statements (select, insert, update, delete
 * etc.), and also demarcate transactions and work with batches. Once you have
 * an SqlMapClient instance, everything you need to work with SQL Maps is easily
 * available.
 * <p/>
 * The SqlMapClient can either be worked with directly as a multi-threaded
 * client (internal session management), or you can get a single threaded
 * session and work with that. There may be a slight performance increase if you
 * explicitly get a session (using the openSession() method), as it saves the
 * SqlMapClient from having to manage threads contexts. But for most cases it
 * won't make much of a difference, so choose whichever paradigm suits your
 * needs or preferences.
 * <p/>
 * An SqlMapClient instance can be safely made <i>static</i> or applied as a
 * <i>Singleton</i>. Generally it's a good idea to make a simple configuration
 * class that will configure the instance (using SqlMapClientBuilder) and
...
public interface SqlMapClient extends SqlMapExecutor, SqlMapTransactionManager {
	 ...
	 * <p/>
	 * <b>Important:</b> Using a user supplied connection basically sidesteps
	 * the datasource so you are responsible for appropriately handling your
	...
	/**
	 * Flushes the data cache that matches the cache model ID provided. cacheId
	 * should include the namespace, even when useStatementNamespaces="false".
	 * 
	 * @param cacheId
	...
	/**
	 * Returns a generated implementation of a cusom mapper class as specified
	 * by the method parameter. The generated implementation will run mapped
	 * statements by matching the method name to the statement name. The mapped
	 * statement elements determine how the statement is run as per the
	 * following:
	 * <ul>
	 * <li>&lt;insert&gt; -- insert()
	 * <li>&lt;update&gt; -- update()
	 * <li>&lt;delete&gt; -- delete()
	 * <li>&lt;select&gt; -- queryForObject, queryForList or queryForMap, as
	 ...
	 * <li>List methodName (Object param [, int skip, int max | , int pageSize])
	 * -- queryForList
	 * <li>Map methodName (Object param, String keyProp [,valueProp]) --
	 * queryForMap
	 * </ul>
	 * 
	 ...
	 * <li>selectXxxxx -- queryForXxxxxx() determined by method signature as
	 * above
	 * <li>queryXxxxx -- queryForXxxxxx() determined by method signature as
	 * above
	 * <li>fetchXxxxx -- queryForXxxxxx() determined by method signature as
	 * above
	 * <li>getXxxxx -- queryForXxxxxx() determined by method signature as above
	 * </ul>
	 * 
	 * @param iface
	 *            The interface that contains methods representing the mapped
