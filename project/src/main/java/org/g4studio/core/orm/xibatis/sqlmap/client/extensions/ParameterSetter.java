 ...
 * TypeHandlerCallback implementations use this interface to process values
 * before they are set on the prepared statement. Each of these methods has a
 * corresponding method on the PreparedStatement class, the only difference
 * being that there is no need to specify the parameter index with these
 * methods.
 * <p/>
