'use strict';
/*
 * ! main.js - v0.1.1 http://admindesigns.com/ Copyright (c) 2015 Admin Designs;
 */

/*
 * Core theme functions required for most of the themes vital functionality
 */
var Core = function(options) {
	...
	var runSideMenu = function(options) {
		...
		var sbOnLoadCheck = function() {
			...
			if ($('body.sb-top').length) {
				// If window is < 1080px wide collapse both sidebars and add
				// ".mobile-view" class
				if ($(window).width() < 900) {
					Body.addClass('sb-top-mobile').removeClass(
							'sb-top-collapsed');
				}
				return;
			}

			// Check Body for classes indicating the state of Left and Right
			// Sidebar.
			// If not found add default sidebar settings(sidebar left open,
			...
			if ($(window).width() < 1080) {
				Body.removeClass('sb-r-o')
						.addClass('mobile-view sb-l-m sb-r-c');
			}
		...
		$('.sidebar-menu-toggle').on(
				...
				function(e) {
					...
					$('.menu-widget').toggleClass('menu-widget-open')
							.slideToggle('fast');
		...
		$('.sidebar-menu li a.accordion-toggle').on(
				...
				function(e) {
					...
					// If the clicked menu item is minified and is a submenu
					// (has sub-nav parent) we do nothing
					if ($('body').hasClass('sb-l-m')
					...
					// If the clicked menu item is a dropdown inside of a
					// dropdown (sublevel menu)
					// we only close menu items which are not a child of the
					// uppermost top level menu
					else {
					...
					// Now we expand targeted menu item, add the ".open-menu"
					// class
	...
	var runHeader = function() {
		...
		$('.navbar-search').on(
				...
				function(e) {
					...
					if ($('body.mobile-view').length
						...
						// Open search bar and add closing icon if one isn't
						// found
						...
						// Fadein remove btn and focus search input on animation
						// complete
						setTimeout(function() {
		...
		$('.dropdown-menu').on(
				...
				function(e) {
					...
					// closes all open multiselect menus. Creates Toggle like
					// functionality
	...
	var runFormElements = function() {
		...
		$('.dropdown-menu .btn-group-nav a').on(
				...
				function(e) {
					...
					// Remove active class from btn-group > btns and toggle tab
					// content
					$(this).siblings('a').removeClass('active').end().addClass(
							'active').tab('show');
				});

		// if btn has ".btn-states" class we monitor it for user clicks. On
		// Click we remove
	...
	var runThemeSettings = function() {
		...
		if ($('#skin-toolbox').length) {
			...
			var settingsObj = {
				// 'headerTone': true,
				//'headerSkin' : '',
				'headerSkin' : 'bg-info', //updated by xiongchun  缺省的样式
				//'sidebarSkin' : '',
				'sidebarSkin' : 'sidebar-light', //updated by xiongchun  缺省的样式
			...
			//TODO 可以考虑支持不同的web配置不通的KEY
			var themeKey = 'myclouds-data-theme';
				...
				$
						.each(
								...
								function(i, e) {
									switch (i) {
									case 'headerSkin':
										Header.removeClass(headerSkins)
												.addClass(e);
										...
										$(
												'#toolbox-header-skin input[value="bg-light"]')
												.prop('checked', false);
										$(
												'#toolbox-header-skin input[value="'
														+ e + '"]').prop(
												'checked', true);
										break;
									case 'sidebarSkin':
										Sidebar.removeClass(sidebarSkins)
												.addClass(e);
										$(
												'#toolbox-sidebar-skin input[value="bg-light"]')
												.prop('checked', false);
										$(
												'#toolbox-sidebar-skin input[value="'
														+ e + '"]').prop(
												'checked', true);
										break;
									case 'headerState':
										if (e === "navbar-fixed-top") {
											...
											// Remove left over inline styles
											// from nanoscroller plugin
											Sidebar.nanoScroller({
												destroy : true
											});
											Sidebar.find('.nano-content').attr(
													'style', '');
			...
			$('#header-option').on(
					...
					function() {
						...
						if (Header.hasClass('navbar-fixed-top')) {
							...
							// Remove left over inline styles from nanoscroller
							// plugin
							Sidebar.nanoScroller({
								destroy : true
							});
							...
							$('#sidebar-option').parent('.checkbox-custom')
									.addClass('checkbox-disabled').end().prop(
											'checked', false).attr('disabled',
											true);
							settingsObj['sidebarState'] = "";
							localStorage.setItem(themeKey, JSON
									.stringify(settingsObj));

							// Remove Fixed Breadcrumb option if navbar isnt
							// fixed
							Breadcrumbs.removeClass('affix');
							$('#breadcrumb-option').parent('.checkbox-custom')
									.addClass('checkbox-disabled').end().prop(
											'checked', false).attr('disabled',
											true);
							...
							$('#sidebar-option').parent('.checkbox-custom')
									.removeClass('checkbox-disabled').end()
									.attr('disabled', false);
							$('#breadcrumb-option').parent('.checkbox-custom')
									.removeClass('checkbox-disabled').end()
									.attr('disabled', false);
			...
			$("#clearLocalStorage").on('click', function() {
				...
				if (bootbox.confirm) {
					bootbox.confirm("确认恢复主题选项到初始配置吗？", function(e) {
						...
						if (e) {
							// Timeout simply gives the user a second for the
							// modal to
							// fade away so they can visibly see the options
							// reset
							setTimeout(function() {
								localStorage.clear();
								location.reload();
	...
	// Fullscreen Panel
	var runFullscreenSettings = function() {

		// If browser is IE we need to pass the fullsreen plugin the 'html'
		// selector
	...
	return {
		init : function(options) {
			...
			runTrays();
			runFormElements();
			//下面这两个方法由demo.js抽取而来 xiongchun
			runThemeSettings();
			runFullscreenSettings();
		}
