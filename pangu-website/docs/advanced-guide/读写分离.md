...
面对日益增加的系统访问量，数据库的吞吐量面临着巨大的挑战。对于同一时刻有大量并发读操作和较少写操作类型的应用系统来说，将数据库拆分为主库和从库搭架主从集群，由主库负责处理事务性的增删改操作，从库负责处理查询操作，能够有效的避免数据更新导致的行锁，从而一定程度提升整个系统的读写并发性能。
...
- **主库**：数据 DML 写操作（新增、修改、删除）使用的数据库。仅支持单主库。
- **从库**：数据 DQL 读操作使用的数据库。可支持多从库。
...
### 技术方案指标对比
### 
|<div style={{width:'120px'}}>技术方案</div>| <div style={{width:'170px'}}>可选组件</div>| 优点 | 缺点
--- | --- | --- | ---
...
<details>
  <summary><b>快速QA：</b>能不能强制把读请求强制路由到主库？</summary>
  <div>
完全可以的。事实上，因为我们的主从复制是异步的，不可避免的会有延迟，所以这个需求也是完全合理的。因此，在对于开启了事务的方法里，框架会自动将此方法事务结束前所有读写请求强制路由到主库上。而且如果不在事务方法里，我们需要把读请求强制路由到主库上，也是可以的。（可参考上述代码中的 <code>readByWriteRoute()</code> 方法的 <code>HintManager</code> API 用法。）
  </div>
</details>
