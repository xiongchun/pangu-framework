<template>
	<el-container>
		<el-aside width="230px">
			<el-container>
				<el-header>
					<el-input placeholder="输入关键字进行过滤" v-model="groupFilterText" prefix-icon="el-icon-Search" clearable></el-input>
				</el-header>
		...
		<el-container>
			<el-header>
				<div class="left-panel">
					...
					<el-button type="primary" plain @click="reset" :disabled="selection.length == 0">重置密码</el-button>
					<el-button type="primary" plain :disabled="selection.length !== 1" @click="grant">选择角色</el-button>
				</div>
	...
	<el-drawer v-model="dialog.info" :size="700" :title="infoTitle" custom-class="drawerBG" direction="rtl" destroy-on-close>
	...
	<el-drawer v-model="dialog.role" :size="650" :title="roleTitle" direction="rtl" destroy-on-close >
		<role-dialog ref="roleDialog" @closeDrawer="dialog.role = false"></role-dialog>
	</el-drawer>
...
export default {
	...
	methods: {
		...
		table_show(row) {
			...
			this.infoTitle = '查看详情 [人员:' + row.name + ']'
			this.$nextTick(() => {
		...
		grant() {
			...
			this.roleTitle = '选择角色 [人员:' + row.name + ']'
			this.$nextTick(() => {
